...
%%
%% Expanded narratives for the nine technical challenges summarised in
...
\textbf{Lesson.} The pallet-first design proved unexpectedly resilient to the January 2026 discontinuation of ink! development (Appendix~\ref{sec:ink-discontinuation}). The decision was made for engineering reasons, but it incidentally conferred robustness against an industry change we could not have foreseen.
...
\textbf{Response.} We documented the constraint as a known limitation, updated benchmark scripts to rotate content items, and treat the 50-child cap as a research-prototype constraint for future work.

\textbf{Lesson.} Bounded FRAME collections trade benchmarkability for capacity limits. The appropriate bound is a trade-off between worst-case weight and maximum expected workload.
