...
The prevailing content economy is unfavorable to creators. A musician earning USD~100,000 in streaming revenue typically receives between USD~55,000 and USD~70,000 after platform fees, payment processing, and distributor commissions. Centralized intermediaries (YouTube, Spotify, Apple Music, Audible, Patreon) deduct 30--45 percent of creator revenue as the price of access to their audiences and infrastructure. Fees are non-negotiable, audiences are non-portable, and content rights remain bound by the platform's terms of service throughout.
...
Existing blockchain alternatives address parts of the problem but not the whole. Non-fungible tokens (NFTs) provide transferable ownership but not recurring access or per-view consumption. Subscription-style decentralized platforms (such as Audius) offer recurring revenue but are confined to a single chain and audience. Cross-chain bridges move assets between ecosystems but lack rights metadata. No existing system, centralized or decentralized, delivers all three monetization models from a single rights primitive portable across blockchains.

The technical primitives to resolve this (cross-chain messaging, programmable smart contracts, on-chain royalty distribution, and NFT-based rights tokenization) all exist as standalone capabilities. Our research examines whether they can be integrated into a coherent system that benefits creators over existing solutions, and the conditions under which creators would adopt it.
